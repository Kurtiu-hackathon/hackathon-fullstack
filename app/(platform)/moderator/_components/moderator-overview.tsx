...
import type { ModeratorSection } from "./moderator-types"

type ModeratorOverviewProps = {
  onNavigate: (section: ModeratorSection) => void
}

export function ModeratorOverview({ onNavigate }: ModeratorOverviewProps) {
  return (
          ...
          <div className="flex flex-col">
            {PENDING_ITEMS_MODERATOR.map((item) => (
              <button
                key={item.label}
                type="button"
                onClick={() => onNavigate(item.section as ModeratorSection)}

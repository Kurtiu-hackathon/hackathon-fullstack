    ...
    <div className="flex flex-col gap-5">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <p className="max-w-lg text-[13.5px] leading-relaxed text-muted-foreground">
          Gerencie usuários e papéis da plataforma. Como Super admin, você pode alterar papéis de qualquer usuário.
        </p>
        <Button
          variant="outline"
          size="sm"
                ...
                {users.map((user) => (
                  <TableRow key={user.id}>
                    ...
                    <TableCell className="text-right">
                      <UserActions user={user} onChanged={refresh} canEditRole />
                    </TableCell>
                  </TableRow>
                ))}
                {loading && (
      ...
      <Blueprint className="p-[18px]">
        <div className="mb-3 flex flex-wrap items-baseline justify-between gap-2.5">
          ...
          <p className="text-[12px] text-primary">Você pode editar esta matriz</p>
        </div>
            ...
            <tbody>
              {PERMISSION_MATRIX.map((row) => (
                <tr key={row.label}>
                  ...
                  <td className="border-b border-border py-2 text-center font-bold text-primary">
                    {row.superAdmin ? "✓" : "—"}
                  </td>
                  <td className="border-b border-border py-2 text-center text-primary">
                    {row.admin ? "✓" : "—"}
                  </td>
                  <td className="border-b border-border py-2 text-center text-muted-foreground">
                    {row.moderator ? "✓" : "—"}
                  </td>
                </tr>
              ))}
